...
class App extends Component {
  ...
  updateRandomImage = x => {
    if (x === 'Yes') {
      const y = imagesList[Math.ceil(Math.random() * imagesList.length - 1)]
      this.setState(prevState => ({randomImage: y, score: prevState.score + 1}))
    } else {
            ...
            <ul className="ul">
              {tabsList.map(each => (
                <TabItem
                  key={each.tabId}
                  tabList={each}
                  isTap={cat === each.tabId}
                  update={this.updateCat}
                />
              ))}
            </ul>
            <ul className="ul2">
              {newList.map(each => (
                <Game
                  key={each.id}
                  list={each}
                  updateRandomImage={this.updateRandomImage}
                  past={randomImage}
                />
              ))}
            </ul>
